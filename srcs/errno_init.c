...
{
  t_errno errnos[] = {{1, "EPERM", "Operation not permitted"},
...
{0, "EUNKNOWN", "Unknown error"}};
    if (!(g_errnos = malloc(sizeof(errnos))))
    {
      ft_putendl_fd("ft_strace: can't malloc errnos names", 2);
      exit(EXIT_FAILURE);
    }
    ft_memcpy(g_errnos, errnos, sizeof(errnos));
}
